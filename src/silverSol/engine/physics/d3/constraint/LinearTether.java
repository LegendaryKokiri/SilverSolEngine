...
public class LinearTether extends Constraint {
	...
	public LinearTether(Volume volumeA, Volume volumeB, Vector3f tetherA, Vector3f tetherB) {
		super();
		this.volumeA = volumeA;
		this.volumeB = volumeB;
		this.tetherA = new Vector3f(tetherA);
		this.tetherB = new Vector3f(tetherB);
		this.tetherLength = Vector3f.sub(volumeA.toGlobalPosition(tetherA), volumeB.toGlobalPosition(tetherB), null).length();
		this.k = DEFAULT_K;
		this.restitution = DEFAULT_RESTITUTION;
	}
	
	public LinearTether(Volume volumeA, Volume volumeB, Vector3f tetherA, Vector3f tetherB, float tetherLength) {
		super();
		this.volumeA = volumeA;
		this.volumeB = volumeB;
		this.tetherA = new Vector3f(tetherA);
	...
	public void resolve(float dt) {
		...
		if(distance < tetherLength) return;
		...
		float ropeImpulse = (restitutionCompoment * Math.max(velocityProjection, 0f)) / massComponent;
		float springImpulse = k * Math.max(distance - tetherLength, 0f) * dt;
